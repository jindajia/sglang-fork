...
# Step 2 for manual TP=1 Hadamard-vs-Hadamard+QR evaluation.
# Run this after launch_hadamard_qr_server_tp1.sh is already running.
#
# Typical usage:
#   MODE_TAG=hadamard_only bash scripts/run_hadamard_qr_eval_tp1.sh
#   MODE_TAG=hadamard_qr   bash scripts/run_hadamard_qr_eval_tp1.sh
...
PRESET_NAME="${PRESET_NAME:-gpqa}"
MODE_TAG="${MODE_TAG:-hadamard_only}"
...
RESULTS_DIR="${RESULTS_DIR:-${REPO_ROOT}/eval_results/hadamard_qr_tp1/${MODEL_SHORT_NAME}/${PRESET_NAME}/${RUN_ID}/${MODE_TAG}}"
LOG_DIR="${LOG_DIR:-${REPO_ROOT}/eval_logs/hadamard_qr_tp1/${MODEL_SHORT_NAME}/${PRESET_NAME}/${RUN_ID}}"
...
validate_env() {
    ...
    case "${MODE_TAG}" in
        hadamard_only|hadamard_qr) ;;
        *)
            echo "Unsupported MODE_TAG='${MODE_TAG}'. Use hadamard_only or hadamard_qr." >&2
            exit 1
            ;;
    esac
}
