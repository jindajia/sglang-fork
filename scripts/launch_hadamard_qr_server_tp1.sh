...
# Step 1 for manual TP=1 Hadamard-vs-Hadamard+QR evaluation.
# Run this in one terminal and keep it alive.
#
# Modes:
#   MODE=hadamard_only -> HADAMARD=1 and no Q rotation
#   MODE=hadamard_qr   -> HADAMARD=1 and enable SGLANG_Q_ROTATION_PATH
...
HADAMARD_ORDER="${HADAMARD_ORDER:-16}"
ROTATE_V="${ROTATE_V:-0}"
...
LOG_DIR="${LOG_DIR:-${REPO_ROOT}/eval_logs/hadamard_qr_tp1/${MODEL_SHORT_NAME}/${RUN_ID}}"
...
validate_env() {
    ...
    case "${MODE}" in
        ...
        *)
            echo "Unsupported MODE='${MODE}'. Use hadamard_only or hadamard_qr." >&2
            exit 1
            ;;
    esac

    if [[ "${MODE}" == "hadamard_qr" ]] && [[ ! -f "${Q_ROTATION_PATH}" ]]; then
...
main() {
    ...
    log "HADAMARD: 1"
    log "HADAMARD_ORDER: ${HADAMARD_ORDER}"
    log "ROTATE_V: ${ROTATE_V}"
    log "Server log: ${SERVER_LOG}"
    if [[ "${MODE}" == "hadamard_qr" ]]; then
