...
MODEL_CONFIGS=(
    "Qwen/Qwen3-8B|1|1|1|0|gpqa_think:5"
    "Qwen/Qwen3-8B|1|1|1|1|humaneval_think:5"
    "Qwen/Qwen3-8B|1|1|1|2|customized_livecodebench_think:5"
    "Qwen/Qwen3-8B|1|1|1|3|aime24_think:5"
    "Qwen/Qwen3-8B|1|1|1|4|math_500_think:5"
)
...
python3 -m sglang.launch_server \
    ...
    --ep "$EP_SIZE" \
    --dp "$DP_SIZE" \
    > /workspace/logs/inference_logs/${MODEL_SHORT_NAME}_${PORT}_server.log 2>&1 &
...
while [ $ELAPSED -lt $MAX_WAIT ]; do
    ...
    if ! kill -0 $SERVER_PID 2>/dev/null; then
        ...
        tail -50 /workspace/logs/inference_logs/${MODEL_SHORT_NAME}_${PORT}_server.log
        exit 1
    fi
...
EOF_SCRIPT
    ...
    echo "$container_script" | sudo docker run -i --rm \
        ...
        --shm-size=32g \
        "$DOCKER_IMAGE" \
        bash -s "$model_name" "$model_short_name" "$CONTAINER_PORT" "$tp_size" "$ep_size" "$dp_size" "$NUM_WORKERS" "$tasks" \
