#!/bin/bash
# seq_eval_kv_rotation.sh — Local SGLang evaluation script (non-Docker)
#
# Hadamard rotation settings are configured per-entry in MODEL_CONFIGS.
#
# Execution logic:
#   Configs are processed in order. For each config:
#     1. Poll nvidia-smi until all required GPUs are free (used memory < GPU_FREE_MEM_MB).
#     2. Launch the eval job in the background.
#     3. Peek at the next config's GPU requirement:
#        - If those GPUs are already free, move on immediately (next config starts right away).
#        - If those GPUs are busy, wait for the current job to finish, sleep 60s for
#          GPU memory to fully release, then continue.
#   This means configs that use different GPUs (e.g. Qwen single-GPU runs) start in
#   parallel, while configs that share GPUs (e.g. MiniMax 8-GPU runs) execute strictly
#   one after another without unnecessary polling.
...
MODEL_CONFIGS=(
    # --- Qwen/Qwen3-4B-Thinking-2507 (TP1, 1 GPU each) ---
    "BASE|0|0|0|BF16|Qwen/Qwen3-4B-Thinking-2507|1|1|1|0|humaneval_think:5"
    "QUANT|1|0|16|INT4|Qwen/Qwen3-4B-Thinking-2507|1|1|1|2|humaneval_think:5"
    "QUANT|1|1|16|INT4|Qwen/Qwen3-4B-Thinking-2507|1|1|1|3|humaneval_think:5"
    "QUANT|1|0|64|INT4|Qwen/Qwen3-4B-Thinking-2507|1|1|1|4|humaneval_think:5"
    "QUANT|1|1|64|INT4|Qwen/Qwen3-4B-Thinking-2507|1|1|1|5|humaneval_think:5"
    "QUANT|1|0|128|INT4|Qwen/Qwen3-4B-Thinking-2507|1|1|1|6|humaneval_think:5"
    "QUANT|1|1|128|INT4|Qwen/Qwen3-4B-Thinking-2507|1|1|1|7|humaneval_think:5"
    "BASE|0|0|0|INT4|Qwen/Qwen3-4B-Thinking-2507|1|1|1|1|humaneval_think:1"
    # --- MiniMaxAI/MiniMax-M2.5 (TP8 EP8 DP1, all 8 GPUs) — BF16 first, INT4 last ---
    "BASE|0|0|0|BF16|MiniMaxAI/MiniMax-M2.5|8|8|1|0,1,2,3,4,5,6,7|${TASKS_ONCE}"
    "QUANT|1|0|16|INT4|MiniMaxAI/MiniMax-M2.5|8|8|1|0,1,2,3,4,5,6,7|${TASKS_ONCE}"
    "QUANT|1|1|16|INT4|MiniMaxAI/MiniMax-M2.5|8|8|1|0,1,2,3,4,5,6,7|${TASKS_ONCE}"
    "QUANT|1|0|64|INT4|MiniMaxAI/MiniMax-M2.5|8|8|1|0,1,2,3,4,5,6,7|${TASKS_ONCE}"
    "QUANT|1|1|64|INT4|MiniMaxAI/MiniMax-M2.5|8|8|1|0,1,2,3,4,5,6,7|${TASKS_ONCE}"
    "QUANT|1|0|128|INT4|MiniMaxAI/MiniMax-M2.5|8|8|1|0,1,2,3,4,5,6,7|${TASKS_ONCE}"
    "QUANT|1|1|128|INT4|MiniMaxAI/MiniMax-M2.5|8|8|1|0,1,2,3,4,5,6,7|${TASKS_ONCE}"
    "BASE|0|0|0|INT4|MiniMaxAI/MiniMax-M2.5|8|8|1|0,1,2,3,4,5,6,7|${TASKS_ONCE}"
)
...
# GPU Free Check
# =============================================================================
# Threshold: GPU is considered free if used memory < GPU_FREE_MEM_MB (default 500 MB)
GPU_FREE_MEM_MB="${GPU_FREE_MEM_MB:-500}"
# How often to poll (seconds)
GPU_POLL_INTERVAL="${GPU_POLL_INTERVAL:-240}"

# Check if all GPUs in a comma-separated list are free.
# Returns 0 if all free, 1 if any is busy.
gpus_are_free() {
    local gpu_list="$1"
    ...
    for gpu_id in "${GPU_IDS[@]}"; do
        ...
        if [ "$used_mb" -ge "$GPU_FREE_MEM_MB" ]; then
            return 1
        fi
    done
    return 0
...
# Block until all specified GPUs are free, then return.
wait_for_gpus_free() {
    local gpu_list="$1"
    local label="$2"
    local waited=0
    while ! gpus_are_free "$gpu_list"; do
        if [ $((waited % 1200)) -eq 0 ]; then
            echo "[$(date '+%Y-%m-%d %H:%M:%S')] Waiting for GPU(s) [$gpu_list] to be free... ($((waited / 60))min elapsed) [$label]"
        fi
...
echo "[$(date '+%Y-%m-%d %H:%M:%S')] SGLang KV Rotation Evaluation (Sequential)"
...
for i in "${!MODEL_CONFIGS[@]}"; do
    ...
    label="${model_short_name}_${rot_suffix} (port=$port, gpu=$gpu_devices)"
    CONFIG_LABELS[$i]="$label"
    EXIT_CODES[$i]=-1

    echo "[$(date '+%Y-%m-%d %H:%M:%S')] [$((i+1))/${N}] Waiting for GPU(s) [$gpu_devices]: $label"

    # Wait until the required GPU(s) are free, then launch in background
    ...
    # If the next config shares any GPU with the current one, block here until the
    # current job finishes (don't rely on GPU memory state which fills up slowly).
    next=$((i + 1))
    if [ "$next" -lt "$N" ]; then
        next_gpu=$(echo "${MODEL_CONFIGS[$next]}" | cut -d'|' -f10)
        overlap=0
        ...
        if [ "$overlap" -eq 1 ]; then
            echo "[$(date '+%Y-%m-%d %H:%M:%S')] Next config overlaps GPU(s) [$next_gpu], waiting for current job to finish first..."
            wait "${PIDS[$i]}"
            EXIT_CODES[$i]=$?
