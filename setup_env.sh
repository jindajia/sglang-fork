#!/bin/bash
# setup_env.sh — One-shot environment setup for zhongzhu_kv (zhongzhu_kv env)
#
# Creates conda env 'zhongzhu_kv' with:
#   - sglang from this repo (editable)
#   - fast-hadamard-transform (compiled with cuda-nvcc 12.8)
#   - tore-eval (editable, for accuracy eval scripts)

set -eo pipefail

SCRIPT_DIR="$(cd "$(dirname "${BASH_SOURCE[0]}")" && pwd)"
CONDA_BASE="/data/$USER/miniconda"
CONDA_ENV_NAME="zhongzhu_kv"
