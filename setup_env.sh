#!/bin/bash
# setup_env.sh — One-shot environment setup for fused_kv_rotation (sglang_env)
#
...
#   - tore-speed-eval (editable, for eval scripts)
...
TORE_EVAL_DIR="$SCRIPT_DIR/tore-speed-eval"
...
TORE_EVAL_BRANCH="jisen/aurora-eval"
current_branch=$(git -C "$TORE_EVAL_DIR" symbolic-ref --short HEAD 2>/dev/null || true)
if [ "$current_branch" != "$TORE_EVAL_BRANCH" ]; then
    echo "  Checking out tore-speed-eval branch '$TORE_EVAL_BRANCH'..."
    git -C "$TORE_EVAL_DIR" checkout "$TORE_EVAL_BRANCH"
fi
echo "  tore-speed-eval branch: $(git -C "$TORE_EVAL_DIR" symbolic-ref --short HEAD)"

echo "=== Installing tore-speed-eval (editable) ==="
if [ ! -f "$TORE_EVAL_DIR/setup.py" ] && [ ! -f "$TORE_EVAL_DIR/pyproject.toml" ]; then
    echo "ERROR: tore-speed-eval pyproject.toml not found after submodule init."
    exit 1
fi
