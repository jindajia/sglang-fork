#!/bin/bash
# setup_env.sh — One-shot environment setup for hetero KV cache eval (sglang_hetero_kv_env)
#
# Creates conda env with:
#   - sglang from this repo (editable)
#   - tore-eval (editable, for eval scripts)
#   - lm_eval (for accuracy benchmarks)

set -eo pipefail

SCRIPT_DIR="$(cd "$(dirname "${BASH_SOURCE[0]}")" && pwd)"
CONDA_BASE="/scratch/$USER/miniconda"
CONDA_ENV_NAME="sglang_hetero_kv_env"
...
echo "=== Installing cuda-nvcc 12.8 (needed for flashinfer JIT) ==="
"$CONDA" install -y -n "$CONDA_ENV_NAME" \
    -c "nvidia/label/cuda-12.8.0" -c nvidia -c conda-forge \
    "cuda-nvcc=12.8.*" "libcublas-dev" -q
